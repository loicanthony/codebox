...
if ismember(Inf,hess)
    hess(hess == Inf) =  max(hess(hess<max(hess(:))));
end
if cond(hess) > 1.d6
    hess = eye(n);
end
...
%     for i = 1:size(failed_points,2)
%         if ~isnan(bb(failed_points(:,i)))
%             idx_to_rm=[idx_to_rm i];
%         end
%     end
%     while j<=i
%         if ismember(j,idx_to_rm)
%             failed_points = failed_points(:,2+k:end);
%             j=j+1;
%         else
%             j=j+1; 
%             k=k+1; %Il faut que le point reste dedans
%         end
%     end
% 
%     if ~isempty(failed_points)
...
global ordo
        % Avec ordonnancement aleatoire
%         vecto = [];
%         for k1 = 1 : pold-1
%            for k2 = k1+1 : pold
%              if sum((test(:,k1)-test(:,k2))) == 1
%                  vecto = [vecto;k1 k2];
%              end
%            end
%         end
    % Portion pour ordonner les points deux à 2
% vecto = [];
% in (1:pold) = false;
% % trie les points données et ressonrt un vecteur vecto
% % des paires de points a ordonner
% for k1 = 1 : pold
%     for k2 = k1+1 : pold
%         if sum((xp1(:,k1)-xp1(:,k2))~=0) == 1
%             vecto = [vecto;k1 k2];
%             in(k1)=true;
%             in(k2)=true;
%             break
...
% % Ordonne les points dans vecto
% final_dirs = [];
% final_order = [];
% 
% 
% for k3 = 1:size(vecto,1)
%     if any(vecto(k3,:)==0)
%         final_order(:,vecto(k3,1)) = vecto(k3,1);
%         final_dirs(:,vecto(k3,1)) = xp1(:,vecto(k3,1));
%     else
%         v2o = [xp1(:,vecto(k3,1)) xp1(:,vecto(k3,2))]; %deux colonne directions
%         switch ordo
%             case 'r'
%                 [~,order] = ior(v2o); % les deux colonnes ordonnées
%             case 'l'
%                 % Avec ordonnancement lexicographique
%                 [~,order] = iol(v2o);
%             case 's'
%                 % Avec ordonnancement par strat�gie du dernier succ�s
%                 [~,order] = ios(v2o,x,complete_history);
%             case 'm'
%                 % Avec ordonnancement avec mod�le
%                 [~, order] = iom(v2o,x,complete_history);
%             case 'o'
%                 % Avec ordonnancement negatif omniscient
%                 low = core_data.fun_data.qbounds(1,1);
%                 dist = core_data.fun_data.dbounds(1);
%                 [~,order] = ioo(low+dist*v2o,'bb');
%             case 'n'
%                 % Avec ordonnancement negatif omniscien
%                 low = core_data.fun_data.qbounds(1,1);
%                 dist = core_data.fun_data.dbounds(1);
%                 [~,order] = ion(low+dist*v2o,'bb');
%         end
%         vec_ordre_ini = [vecto(k3,1) vecto(k3,2)];
%         vec_ordre_fin = vec_ordre_ini(:,order);
%         final_order(vecto(k3,1)) = vec_ordre_fin(1);
%         final_order(vecto(k3,2)) = vec_ordre_fin(2);
%     end
% end
% [xp1, order] = ior(xp1);
switch ordo
    case 'r'
        [~,order] = ior(xp1); % les deux colonnes ordonnées
    case 'l'
        % Avec ordonnancement lexicographique
        [~,order] = iol(xp1);
    case 's'
        % Avec ordonnancement par strat�gie du dernier succ�s
        [~,order] = ios(xp1,x,complete_history);
    case 'm'
        % Avec ordonnancement avec mod�le
        [~, order] = iom(xp1,x,complete_history);
    case 'o'
        % Avec ordonnancement negatif omniscient
        low = core_data.fun_data.qbounds(1,1);
        dist = core_data.fun_data.dbounds(1);
        [~,order] = ioo(low+dist*xp1,'bb');
    case 'n'
        % Avec ordonnancement negatif omniscien
        low = core_data.fun_data.qbounds(1,1);
        dist = core_data.fun_data.dbounds(1);
        [~,order] = ion(low+dist*xp1,'bb');
end
xp1 = xp1(:,order);
dx1=dx1(:,order);
